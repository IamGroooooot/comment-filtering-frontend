...
public class StartSceneManger : MonoBehaviour
{
    ...
    public void SignInBtnClicked()
    {
        ...
        if(PlayerInfoManager.id.Length > 0 && PlayerInfoManager.pw.Length > 0 && PlayerInfoManager.reg_id == GameObject.Find("InputField_ID").GetComponent<InputField>().text && PlayerInfoManager.reg_pw == GameObject.Find("InputField_PW").GetComponent<InputField>().text)
